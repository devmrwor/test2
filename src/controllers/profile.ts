...
export async function getProfileById(id: number): Promise<Model<IProfile>> {
  ...
  const includeCategory: Includeable = {
  ...
  const profile = await models.Profile.findByPk(id, {
    include: [includeTranslations, includeStatistic, includeRatings, includeUser, includeCategory],
  });
  if (!profile) {
...
export async function getProfileByUserId(id: string, isMain = false): Promise<Model<IProfile>> {
...
export async function getMainProfileByUserId(id: string): Promise<Model<IProfile>> {
...
export async function updateUserProfile(
  ...
  if (!profile.get('is_main') && language && profile.get('profile_language') !== language) {
    const filteredFields = ['name', 'surname', 'description'];
    ...
    await updateOrCreateProfileTranslation(id, language, filteredData);
  }

  parseProfileBody(data as IProfile);
  const updatedProfile = await profile.update(data);
  return updatedProfile;
}

export async function updateOrCreateProfileTranslation(
  profileId: number,
  language: ProfileLanguages,
  data: Partial<IProfile>
) {
  const profileTranslation = await models.ProfileTranslation.findOne({
    where: { profile_id: profileId, language },
  });

  if (profileTranslation) {
    await profileTranslation.update(data);
  } else {
    await models.ProfileTranslation.create({ ...data, profile_id: profileId, language });
  }
}

export const parseProfileBody = (body: IProfile) => {
  try {
    if (body.services_pricelist) {
      const services_pricelist =
        typeof body.services_pricelist === 'string'
          ? JSON.parse(body.services_pricelist as string)
          : body.services_pricelist;
      if (!services_pricelist) return;
      const highest_price = services_pricelist?.sort((a, b) => b.price - a.price)[0]?.price ?? null;
      const lowest_price = services_pricelist?.sort((a, b) => a.price - b.price)[0]?.price ?? null;

      body.highest_price = highest_price;
      body.lowest_price = lowest_price;
    }

    if (body.languages) {
      const languages = typeof body.languages === 'string' ? JSON.parse(body.languages as string) : body.languages;
      const languageCodes = languages.map((item: any) => item.code);
      const concatenatedLanguageCodes = `/${languageCodes.join('/')}/`;
      body.languages_codes = concatenatedLanguageCodes;
    }

    // if (body.portfolio_photos) {
    //   body.portfolio_photos = JSON.stringify(body.portfolio_photos);
    // }
  } catch (e) {
    console.log(e);
  }
...
export async function createProfile(id: number, data: Partial<IProfile>): Promise<Model<IProfile>> {
  await getUserById(id);

  parseProfileBody(data as IProfile);

  const profile = await models.Profile.create({ ...data, user_id: id });
  return profile;
}

const createDefaultUserFromProfile = async (profile: any, email: string) => {
  const user = await createUser({
    email,
    password: profile.user_password || (process.env.NEXT_PUBLIC_DEFAULT_PASSWORD as string),
    role: Roles.EXECUTOR,
  ...
  await createProfileStatistic(user.id);

  return user;
};

export async function createMultipleProfiles(data: IProfile[], isTranslation = false): Promise<Model<IProfile[]>> {
  const profiles: Model<IProfile>[] = [];
...
export async function getAllProfiles(
  ...
  if (sortField === 'tags') {
    ...
    order.push([Sequelize.fn('array_length', Sequelize.col('portfolio_photos'), 1), dir]);
  } else if (field && dir) {
...
export const getProtectedProfiles = (
  role: Roles,
  page: number = DEFAULT_PAGE,
  limit: number = DEFAULT_LIMIT,
  customerType: CustomerTypes = CustomerTypes.ALL,
  ...
  return getAllProfiles(page, limit, customerType, searchText, sortField, sortOrder, filter);
};
...
export async function getCustomers({
  ...
  sortField,
  sortOrder,
}: CustomerControllerOptions): Promise<IPaginationResponse<IProfile>> {
  const offset = Math.max(0, (page - 1) * limit);
  ...
  const profiles = await models.User.findAndCountAll({
    where,
    limit,
    offset,
    order,
...
export const toggleProfileMessenger = async (id: number, messengerId: number, nickname: string) => {
  const profile = await getProfileById(id);

  let messengers: MessengerItem[] = JSON.parse(JSON.stringify(profile.get('messengers')));
  if (typeof messengers === 'string') {
    messengers = JSON.parse(messengers);
  }

  const messengerIndex = messengers.findIndex(
    (messenger) => messenger.messenger.id === messengerId && nickname === messenger.nicknameOrNumber
  );
  const messenger = messengers[messengerIndex];
  if (!messenger) throw new Error('Messenger not found');

  messenger.messenger.show_others = !messenger.messenger.show_others;

  messengers.splice(messengerIndex, 1, messenger);

  await profile.update({ messengers: messengers });

  return messenger;
};

export const addMessengersToProfile = async (id: number, data: MessengerItem[]) => {
  const profile = await getProfileById(id);

  const messengers: MessengerItem[] = [...profile.get('messengers'), ...data];
